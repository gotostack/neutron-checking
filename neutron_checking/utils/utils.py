...
from neutron_checking._i18n import _, _LE
from neutron_checking.db import api as db_api

TIME_FORMAT = "%Y-%m-%dT%H:%M:%SZ"
LOG = logging.getLogger(__name__)
SYNCHRONIZED_PREFIX = 'neutron-checking-'
...
def import_modules_recursively(topdir):
    ...
    for root, dirs, files in os.walk(topdir):
        for file_ in files:
            ...
            prefixlen = len(os.path.dirname(neutron_checking.__file__))
            import_base = 'neutron_checking' + import_base[prefixlen:]
